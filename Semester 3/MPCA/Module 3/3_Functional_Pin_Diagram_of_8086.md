...
> The **Intel 8086** is a 40-pin IC. To save pin count, it uses **multiplexed buses** (pins share dual roles based on the clock cycle). A unique feature is the `MN/MX (Pin 33)'` pin, which toggles the processor between **Minimum Mode** (single-processor) and **Maximum Mode** (multi-processor).
...
### 3. Minimum Mode Pins (MN/MX (Pin 33)' tied to +5V)
...
### 4. Maximum Mode Pins (MN/MX (Pin 33)' tied to GND)
...
> 4. Can operate in **Minimum Mode** (standalone) or **Maximum Mode** (multi-processor) using the `MN/MX (Pin 33)'` pin.
